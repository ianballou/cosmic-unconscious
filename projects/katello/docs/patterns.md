...
## EVR comparison architecture — three parallel paths

Katello has three independent mechanisms for comparing RPM versions in SQL/Ruby.
They exist because of historical evolution and performance trade-offs:

### 1. `evr_t` composite type + Postgres array comparison
- **Defined in:** `20240924161240_katello_recreate_evr_constructs.rb` (previously
  `20200213184848_create_evr_type.rb` which used `CREATE EXTENSION evr`)
- **Type:** `evr_t = (epoch INT, version evr_array_item[], release evr_array_item[])`
- **Populated by:** `evr_trigger()` on INSERT/UPDATE of `katello_rpms` and
  `katello_installed_packages` — calls `rpmver_array()` to parse version/release text
- **Used by:** Applicability calculation (`applicable_content_helper.rb:91` — `evr > evr`),
  upgrade dropdown (`host_package_presenter.rb` — `ORDER BY evr DESC`),
  host upgrade logic (`host_managed_extensions.rb` — `ORDER BY evr DESC`)
- **Index:** `(name, arch, evr)` on `katello_rpms`

### 2. `version_sortable` / `release_sortable` text columns
- **Generated by:** `Katello::Util::Package.sortable_version()` — encodes version
  segments as length-prefixed strings (e.g. `"3.9"` → `"01-3.01-9"`, alpha segments
  get `$` prefix) for lexicographic COLLATE "C" comparison
- **Used by:** `Rpm.latest()` LEFT OUTER JOIN, `Rpm.default_sort`,
  `PackageFilter`, `scoped_search_evr_compare`, `scoped_search_sortable`

### 3. Python/Ruby-side comparison (not currently in Katello)
- Pulp moved to this approach: load packages into memory, sort with `RpmVersion`
  objects that implement the full `rpmvercmp` algorithm including tilde/caret
- Katello does NOT currently have a Ruby equivalent of `rpmvercmp`

**Key insight:** The applicability query (`evr > installed_evr`) is a SQL WHERE clause
joining two tables, not just an ORDER BY. It cannot be moved to Ruby without loading
all candidate packages into memory — a performance non-starter. Any fix must work
at the SQL level.
